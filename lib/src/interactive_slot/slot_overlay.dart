...
class _SlotOverlayState extends State<SlotOverlay> {
  ...
  Widget _buildColumnLayout(CalendarSlot slot, Color accent, bool isDragging) {
    ...
    final gStartMinute = (anchor?.startDateTime.totalMinutes ?? rStartMinute)
        .toDouble();
    final gEndMinuteAbs =
        (gStartMinute + (anchor?.durationInMinutes ?? slot.durationInMinutes))
            .toDouble();
    ...
    for (int d = 0; d < rTotalDays; d++) {
      ...
      if (isFirst && isLast) {
        ...
        if (gapH > 0 &&
            ...
            rEndMinuteAbs % PlannerTimeMapper.minutesPerHour == 0)
          raw -= gapH;
        ...
        final intra =
            rEndMinuteAbs - (rTotalDays - 1) * PlannerTimeMapper.minutesPerDay;
        double raw = widget.timeMapper.minuteToY(intra);
        if (gapH > 0 &&
            intra > 0 &&
            intra < PlannerTimeMapper.minutesPerDay &&
            intra % PlannerTimeMapper.minutesPerHour == 0)
          raw -= gapH;
        segTop = 0;
      ...
      for (int d = 0; d < gTotalDays; d++) {
        ...
        if (gIsFirst && gIsLast) {
          ...
          if (gapH > 0 &&
              ...
              gEndMinuteAbs % PlannerTimeMapper.minutesPerHour == 0)
            raw -= gapH;
    ...
    return Positioned(
      ...
      width: totalWidth,
      height: dayHeight,
      child: Stack(clipBehavior: Clip.none, children: outerChildren),
    );
  }
